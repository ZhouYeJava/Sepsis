...
def construct_all_file(data):
	part_data = random.sample(data, int(0.005*len(data))) #sub-sampling
	data_string = [d.all_select() for d in part_data]
	return "".join(data_string)
...
if __name__=="__main__":
	...
	for i in range(1, num_fold+1):
		...
		train_file = construct_all_file(train)
		...
		for time in range(t0[0], t0[1]+1):
			...
			test_file = construct_single_file(test, time)
