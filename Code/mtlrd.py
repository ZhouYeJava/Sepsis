	...
	def __eq__(self, other):
		if not isinstance(other, self.__class__):
			...
			if self.identifier==other.identifier:
				...
				return False
	def group_select(self, start, end):
		output = ""
		for d in self.data:
			if d>=start and d<=end:
				output += str(self.shock_time-d)+" "+str(self.flag)+" "+self.data[d]+"\n"
		return output
...
def construct_group_file(data, start, end):
	part_data = random.sample(data, int(0.01*len(data))) #sub-sampling
	data_string = [d.group_select(start, end) for d in part_data]
...
if __name__=="__main__":
	...
	t0 = [int(x) for x in sys.argv[4].split(",")] #current time points (features)
	...
	for i in range(1, num_fold+1):
		...
		train_file = construct_group_file(train, t0[0], t0[1])
		...
		brier_fold = 0
		accuracy_fold = 0
		for time in range(t0[0], t0[1]+1):
			writer_test = open("fold_"+str(i)+".test", "w")
			writer_test.write(construct_single_file(test, time))
			writer_test.close()
			os.system("./mtlr_test -l class -t "+str(t1-time)+" -m "+str(t_max)+" -i "+"fold_"+str(i)+".test"+" -o "+"fold_"+str(i)+".model > fold_"+str(i)+".result")
			score = parse_result("fold_"+str(i)+".result")
			brier_fold += score[0]
			accuracy_fold += score[1]
		brier.append(brier_fold/(t0[1]+1-t0[0]))
		accuracy.append(accuracy_fold/(t0[1]+1-t0[0]))
