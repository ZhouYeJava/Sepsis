...
test_evaluation <- function(shock_test, normal_test, model) {
	test <- foreach (patient = c(shock_test, normal_test), .combine="rbind") %dopar% {
		data <- patient$data
		prediction <- as.data.frame(predict(object=model, newx=data[,-1], proba=T)$proba)
...
model_selection <- function(shock_patients, normal_patients, candicates) {
	...
	for (j in 1:length(candicates)) {
		model <- LiblineaR(data=training[,-ncol(training)], labels=as.factor(training[,ncol(training)]), cost=candicates[j], type=0)
...
for (i in 1:num_fold) {
	...
	parameter <- model_selection(shock_patients, normal_patients, penalty)
	print(paste("parameter = ", parameter, sep=""))
	training <- construct_train(shock_train, normal_train)
	model <- LiblineaR(data=training[,-ncol(training)], labels=as.factor(training[,ncol(training)]), cost=parameter, type=0)
